...
import { pushSingleton, mirrorList, pushDelete } from "./cms-sync";
...
export const cmsStore = {
  ...
  upsertPage(p: Partial<CmsPage> & { id?: string }): CmsPage {
    ...
    write(K_PAGES, next);
    // Mirror the whole set so the backend always holds every page (incl. defaults).
    mirrorList("pages", next, "pages");
    return created;
  },
  deletePage(id: string) {
    write(K_PAGES, this.listPages().filter(p => p.id !== id));
    pushDelete("pages", id, "pages");
  },
  ...
  saveNav(items: NavItem[]) {
    write(K_NAV, items);
    pushSingleton("nav", "nav", { items }, "nav");
  },
  resetNav() {
    write(K_NAV, DEFAULT_NAV);
    pushSingleton("nav", "nav", { items: DEFAULT_NAV }, "nav");
  },
  newId: id,
};
...
// ---------- Backend hydration ----------
let hydrated = false;
/** Pull server-stored pages + nav into the local cache (server wins). Runs once. */
export async function hydrateCms(): Promise<void> {
  if (!isBrowser() || hydrated) return;
  hydrated = true;
  seed();
  try {
    const [pages, navRows] = await Promise.all([
      apiClient.list<CmsPage>("pages"),
      apiClient.list<{ id: string; items?: NavItem[] }>("nav"),
    ]);
    if (Array.isArray(pages) && pages.length) write(K_PAGES, pages);
    const navItems = navRows[0]?.items;
    if (Array.isArray(navItems) && navItems.length) write(K_NAV, navItems);
  } catch {
    /* API offline — keep cache/defaults so the public site still renders */
  }
}
if (isBrowser()) void hydrateCms();
