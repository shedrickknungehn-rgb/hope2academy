...
export const brandStore = {
  ...
  set(patch: Partial<BrandSettings>) {
    ...
    write(next);
    pushSingleton("brand", "brand", next, "brand");
    return next;
  },
  reset() {
    write(DEFAULT_BRAND);
    pushSingleton("brand", "brand", DEFAULT_BRAND, "brand");
    return DEFAULT_BRAND;
  },
};
...
// ---------- Backend hydration ----------
let hydrated = false;
/** Pull server-stored brand into the local cache (server wins). Runs once. */
export async function hydrateBrand(): Promise<void> {
  if (!isBrowser() || hydrated) return;
  hydrated = true;
  try {
    const rows = await apiClient.list<BrandSettings & { id?: string }>("brand");
    const remote = rows[0];
    if (remote) {
      const { id: _id, ...rest } = remote as BrandSettings & { id?: string };
      write({ ...DEFAULT_BRAND, ...rest });
    }
  } catch {
    /* API offline — keep cache/defaults so the public site still renders */
  }
}
if (isBrowser()) void hydrateBrand();
